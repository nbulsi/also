...
struct xor_and_mux_storage_data
{
  uint32_t num_pis = 0u;
  uint32_t num_pos = 0u;
...
/*! \brief xor_and_mux storage container

  xor_and_muxs have nodes with fan-in 3.  We split of one bit of the index pointer to
...
using xor_and_mux_storage = storage<regular_node<3, 2, 1>,
                                    xor_and_mux_storage_data>;

class xor_and_mux_network
{
public:
#pragma region Types and constructors
  ...
  using base_type = xor_and_mux_network;
  using storage = std::shared_ptr<xor_and_mux_storage>;
  ...
  struct signal
  {
    ...
    signal( xor_and_mux_storage::node_type::pointer_type const& p )
    ...
    operator xor_and_mux_storage::node_type::pointer_type() const
    {
  ...
  xor_and_mux_network()
      : _storage( std::make_shared<xor_and_mux_storage>() ),
        _events( std::make_shared<decltype( _events )::element_type>() )
  {
  }

  xor_and_mux_network( std::shared_ptr<xor_and_mux_storage> storage )
      : _storage( storage ),
        _events( std::make_shared<decltype( _events )::element_type>() )
  {
  }
...
#pragma region Create binary / ternary functions
  ...
  signal create_mux( signal a, signal b, signal c )
  {
    ...
    if ( a.index == b.index == c.index )
    {
      ...
      {
        return ( a.complement == b.complement ) ? get_constant( true ) : get_constant( false );
      }
    }
  ...
  signal create_and( signal a, signal b, signal c = {0, 1} )
  {
    if ( a.index < b.index )
    {
      std::swap( a, b );
...
#pragma region Create nary functions
  ...
  {
    return ternary_tree_reduce( fs.begin(), fs.end(), get_constant( true ), [this]( auto const& a, auto const& b, auto const& c = {0, 1} ) { return create_and( a, b ); } );
  }
...
#pragma region Create arbitrary functions
  signal clone_node( xor_and_mux_network const& other, node const& source, std::vector<signal> const& children )
  {
    assert( ( children.size() == 3u ) || ( children.size() == 2u ) );

    if ( other.is_mux( source ) )
    {
      return create_mux( children[0u], children[1u], children[2u] );
    }
    else if ( other.is_xor3( source ) )
    {
      return create_xor3( children[0u], children[1u], children[2u] );
    }
    else if ( other.is_and( source ) )
    {
...
#pragma region Restructuring
  std::optional<std::pair<node, signal>> replace_in_node( node const& n, node const& old_node, signal new_signal )
  {
    ...
    auto _is_mux = is_mux( n );
    auto _is_and = is_and( n );
    auto _is_xor3 = is_xor3( n );
    /* normalize order */
    if ( _is_and )
    {
      child2 = get_constant( true );
    ...
    if ( _is_xor3)
    {
      ...
      assert( child1.index >= child0.index );
      assert( child1.index >= child2.index );
    }
    ...
    if ( _is_mux )
    {
      if ( child0.index == 0 )
      {
        return std::make_pair( n, child0.complement ? child2 : child1 );
      }
      if ( child0.index == child1.index == child2.index )
      {
        const auto node_pol = child1.complement != child2.complement;
        const auto diff_pol = child0.complement != child1.complement;
        if ( !node_pol )
          return std::make_pair( n, child2 );
        else
          return std::make_pair( n, diff_pol ? get_constant( false ) : get_constant( true ) );
      }
    }
     if ( _is_and )
    {
      ...
      else if ( child1.index == 0 )
      {
        return std::make_pair( n, child1.complement ? child0 : get_constant( false ) );
      }
    }
    if ( _is_xor3)
    {
...
#pragma region Structural properties
  ...
  {
    return n > 0 && !is_ci( n ) && ( _storage->nodes[n].children[0].index > _storage->nodes[n].children[1].index ) && ( _storage->nodes[n].children[2].index == 0u );
  }
  ...
  {
    return n > 0 && !is_ci( n ) && (_storage->nodes[n].children[0].index < _storage->nodes[n].children[1].index < _storage->nodes[n].children[2].index);
  }

  bool is_xor3( node const& n ) const
  {
    return n > 0 && !is_ci( n ) && ( _storage->nodes[n].children[0].index < _storage->nodes[n].children[1].index ) && ( _storage->nodes[n].children[1].index > _storage->nodes[n].children[2].index );
  }
#pragma endregion
...
#pragma region Functional properties
  ...
  {
    if ( ( _storage->nodes[n].children[2].index == 0u ) && ( _storage->nodes[n].children[2].weight == true) )
    {
      kitty::dynamic_truth_table _tt( 2 );
      _tt._bits[0] = 0x8;
      return _tt;
    }
    else
    {
      ...
      _tt._bits[0] = is_xor3( n ) ? 0x96 : 0xd8;
      return _tt;
    }
  }
#pragma endregion
...
#pragma region Value simulation
  ...
  compute( node const& n, Iterator begin, Iterator end ) const
  {
    ...
    {
      return ( v1 ^ c1.weight ) ? ( v2 ^ c2.weight ) : ( v3 ^ c3.weight );
          //( ( v1 ^ c1.weight ) && ( v2 ^ c2.weight ) ) || ( ( v3 ^ c3.weight ) && ( ~( v1 ^ c1.weight ) ) );
    }
    else
    {
  ...
  compute( node const& n, Iterator begin, Iterator end ) const
  {
    ...
    {
      return kitty::ternary_ite( c1.weight ? ~tt1 : tt1, c2.weight ? ~tt2 : tt2, c3.weight ? ~tt3 : tt3 );
    }
    else
    {
...
public:
  std::shared_ptr<xor_and_mux_storage> _storage;
...
template<>
struct hash<mockturtle::xor_and_mux_network::signal>
{
  uint64_t operator()( mockturtle::xor_and_mux_network::signal const& s ) const noexcept
  {
    uint64_t k = s.data;
    k ^= k >> 33;
