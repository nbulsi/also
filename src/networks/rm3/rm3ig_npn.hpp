...
class rm3ig_npn_resynthesis
{
public:
  // 默认构造函数，调用 build_db() 来构建内部的数据库
  rm3ig_npn_resynthesis()
  {
    build_db();
  }
  ...
  void operator()( rm3_network& rm3ig,
                   ...
                   Fn&& fn ) const
  {
    ...
    std::vector<rm3_network::signal> pis( 4, rm3ig.get_constant( false ) );
    std::copy( begin, end, pis.begin() );

    std::vector<rm3_network::signal> pis_perm( 4 );
    ...
    for ( auto const& po : it->second )
    {
      ...
      auto f = cleanup_dangling( topo, rm3ig, pis_perm.begin(), pis_perm.end() ).front();
...
private:
  // 存储从文件加载的最优 rm3ig 结构
  std::unordered_map<std::string, std::vector<std::string>> opt_rm3ig;
  ...
  void load_optimal_rm3ig()
  {
    ...
    while ( std::getline( infile, line ) )
    {
      ...
      opt_rm3ig.insert( std::make_pair( strs[0], v ) );
    }
  }

  // 根据输入的字符串向量和信号向量创建 rm3ig 网络
  std::vector<rm3_network::signal> creat_rm3ig_from_str_vec( const std::vector<std::string> strs,
                                                             const std::vector<rm3_network::signal>& signals )
  {
    auto sig = signals;
    auto size = strs.size();
    auto pol = std::stoi( strs[0] );

    std::vector<rm3_network::signal> result;
    ...
    for ( auto i = 1; i < size; i++ )
    {
      ...
      {
      case 0:
        sig.push_back( db.create_rm3( sig[a], sig[b], sig[c] ) );
        break;

      case 1:
        sig.push_back( db.create_rm3( sig[a] ^ true, sig[b], sig[c] ) );
        break;

      case 2:
        sig.push_back( db.create_rm3( sig[a], sig[b] ^ true, sig[c] ) );
        break;

      case 3:
        sig.push_back( db.create_rm3( sig[a], sig[b], sig[c] ^ true ) );
        break;
  ...
  void build_db()
  {
    std::vector<rm3_network::signal> signals;
    ...
    for ( const auto e : opt_rm3ig )
    {
      //std::cout << "tt" << e.first << std::endl;
      if ( e.first == "0x0000" )
      {
        std::vector<rm3_network::signal> tmp{ signals[0] };
        //std::cout << "signals[0]" << signals[0] << std::endl;
        class2signal.insert( std::make_pair( e.first, tmp ) );
      }
      else if ( e.first == "0x00ff" )
      {
        std::vector<rm3_network::signal> tmp{ signals[4] ^ true };
        class2signal.insert( std::make_pair( e.first, tmp ) );
      }
      else
      {
        ...
        class2signal.insert( std::make_pair( e.first, creat_rm3ig_from_str_vec( e.second, signals ) ) );
      }
    }
  }
  ...
  std::unordered_map<std::string, std::vector<rm3_network::signal>> class2signal;
};
} // namespace mockturtle