...
private:
  ...
  struct node_cost
  {
    ...
    void parser_csv( const string& csv )
    {
      ...
      std::cout << fmt::format( "[LIB] MUX area {} / delay {}, AND area {} / delay {}, NOT area {} / delay {}\n",
                                  mux_area, mux_delay, and_area, and_delay, not_area, not_delay );
    }
  ...
  std::tuple<unsigned, unsigned, unsigned> split_critical_path( Ntk const& ntk )
  {
    ...
    while ( !ntk.is_constant( cp_node ) && !ntk.is_pi( cp_node ) )
    {
      ...
      {
        num_mux++;
        node_level = int( node_fn.mux_delay * node_fn.times );
      }
      else
      {
        assert( false && "only support MAG now" );
