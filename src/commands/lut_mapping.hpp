...
public:
  explicit lut_mapping_command( const environment::ptr& env ) : command( env, "LUT mapping : using AIG as default" )
  {
    ...
    add_flag( "--seq, -q", "LUT mapping for sequential AIG (SeqAIG -> SeqLUT)" );
...
protected:
  void execute()
  {
    ...
    if ( is_set( "seq" ) )
    {
      ...
      if ( store<seq_aig_network>().size() == 0 )
      {
        std::cerr << "[e] no sequential AIG in the store\n";
        return;
      }

      seq_aig_network seq_aig = store<seq_aig_network>().current();

      std::cout << fmt::format( "[i] SeqAIG before mapping: {} PIs, {} POs, {} regs, {} gates\n",
      ...
      auto seq_klut = *collapse_mapped_network<seq_klut_network>( mapped_seq_aig );

      std::cout << fmt::format( "[i] SeqLUT after mapping: {} PIs, {} POs, {} regs, {} LUTs\n",
