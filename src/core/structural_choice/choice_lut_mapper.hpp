#ifndef CHOICE_LUT_MAPPER
#define CHOICE_LUT_MAPPER
#pragma once
...
struct choice_lut_map_params
{
  choice_lut_map_params()
  {
    cut_enumeration_ps.cut_size = 6u;
    cut_enumeration_ps.cut_limit = 32u;
  ...
  uint32_t sop_cut_limit{ 10 };
};

struct choice_lut_map_stats
{
  /*! \brief Area result. */
  uint32_t area{ 0 };
...
/* cut data */
struct choice_cut_enumeration_lut_cut
{
  uint32_t delay{ 0 };
  uint32_t lut_area{ 0 };
...
class choice_lut_cut_set
{
public:
  /*! \brief Standard constructor.
   */
  choice_lut_cut_set() { clear(); }

  /*! \brief Assignment operator.
   */
  choice_lut_cut_set& operator=( choice_lut_cut_set const& other )
  {
    if ( this != &other )
    {
  ...
  void merge( const choice_lut_cut_set<CutType, MaxCuts>& other )
  {
    for ( const auto& cut_ptr : other )
    {
      this->simple_insert( *cut_ptr, lut_cut_sort_type::AREA );
    }
  }
   ...
   */
  static bool compare( CutType const& cut1, CutType const& cut2,
                       lut_cut_sort_type sort = lut_cut_sort_type::NONE )
  {
  ...
  void simple_insert( CutType const& cut,
                      lut_cut_sort_type sort = lut_cut_sort_type::NONE )
  {
    ...
    if ( sort == lut_cut_sort_type::DELAY )
    {
      ipos = std::lower_bound( _pcuts.begin(), _pend, &cut, []( auto a, auto b )
                               { return sort_delay( *a, *b ); } );
    }
    else if ( sort == lut_cut_sort_type::DELAY2 )
    {
      ipos = std::lower_bound( _pcuts.begin(), _pend, &cut, []( auto a, auto b )
                               { return sort_delay2( *a, *b ); } );
    }
    else if ( sort == lut_cut_sort_type::AREA )
    {
      ipos = std::lower_bound( _pcuts.begin(), _pend, &cut,
                               []( auto a, auto b )
                               { return sort_area( *a, *b ); } );
    }
    else if ( sort == lut_cut_sort_type::AREA2 )
    {
      ipos = std::lower_bound( _pcuts.begin(), _pend, &cut, []( auto a, auto b )
                               { return sort_area2( *a, *b ); } );
    }
    else /* NONE */
    {
      ipos == _pend;
  ...
  void insert( CutType const& cut, bool skip0 = false,
               lut_cut_sort_type sort = lut_cut_sort_type::NONE )
  {
    ...
    _pcend = _pend = std::stable_partition(
        begin, _pend,
        [&cut]( auto const* other )
  ...
  void replace( uint32_t index, CutType const& cut ) { *_pcuts[index] = cut; }
...
public:
  static constexpr uint32_t max_cut_num = 200;
  static constexpr uint32_t max_cut_size = 16;
  static constexpr uint32_t max_cubes = 64;
  static constexpr uint32_t max_sop_decomp_size =
      max_cut_size * ( max_cubes + 1 );
  using cut_t = cut<max_cut_size,
                    cut_data<StoreFunction, choice_cut_enumeration_lut_cut>>;
  using cut_set_t = choice_lut_cut_set<cut_t, max_cut_num>;
  std::unordered_map<uint32_t, cut_set_t> equiv_node_cuts;
  ...
  using cubes_queue_t = std::priority_queue<uint32_t, std::vector<uint32_t>,
                                            std::greater<uint32_t>>;
  using lut_info = std::pair<kitty::dynamic_truth_table,
                             std::vector<mockturtle::signal<klut_network>>>;

public:
  explicit choice_lut_map_impl( Ntk& ntk, choice_lut_map_params const& ps,
                                choice_lut_map_stats& st )
      : ntk( ntk ), ps( ps ), st( st ), node_match( ntk.size() ), cuts( ntk.size() )
  {
    assert( ps.cut_enumeration_ps.cut_limit < max_cut_num &&
...
private:
  void perform_mapping()
  {
    ...
    lut_cut_sort_type area_sort =
        ( ps.area_oriented_mapping && !ps.edge_optimization )
            ? lut_cut_sort_type::AREA
            : lut_cut_sort_type::AREA2;
    ...
    ntk.foreach_node( [this]( auto const& n )
                      {
                        ...
                        // if (ps.insert && ntk.is_repr(n)) {
                        //   auto choice_node = ntk.get_equiv_node(n);
                        //   auto choice_index = ntk.node_to_index(choice_node);
                        //   while (choice_node < ntk.size()) {
                        //     node_match[choice_index].map_refs = node_data.map_refs;
                        //     node_match[choice_index].est_refs = node_data.est_refs;
                        ...
                        bool recompute_cuts )
  {
    ...
    for ( auto const& n : topo_order )
    {
      if constexpr ( !ELA )
      {
        ...
        {
          node_match[index].est_refs = ( 2.0 * node_match[index].est_refs +
                                         1.0 * node_match[index].map_refs ) /
                                       3.0;
        }
        else
        {
          node_match[index].est_refs =
              static_cast<float>( node_match[index].map_refs );
        }
      }
      ...
      if ( recompute_cuts )
      {
        if constexpr ( Ntk::min_fanin_size == 2 && Ntk::max_fanin_size == 2 )
        {
          ...
          if ( ps.insert && ntk.is_repr( n ) )
          {
            compute_choice_cut<DO_AREA, ELA>( n, sort, preprocess );
          }
        }
        else
        {
    ...
    /* round stats */
    {
      ...
      if ( ( sort == lut_cut_sort_type::AREA ||
             sort == lut_cut_sort_type::AREA2 ) &&
           ELA )
      {
        stats << fmt::format(
            "[i] Area     : Delay = {:8d}  Area = {:8d}  Edges = {:8d}  Cuts "
            "= "
            "{:8d}\n",
      ...
      {
        stats << fmt::format(
            "[i] AreaFlow : Delay = {:8d}  Area = {:8d}  Edges = {:8d}  Cuts "
            "= "
            "{:8d}\n",
            delay, area, edges, cuts_total );
      }
      ...
      {
        stats << fmt::format(
            "[i] Delay2   : Delay = {:8d}  Area = {:8d}  Edges = {:8d}  Cuts "
            "= "
            "{:8d}\n",
            delay, area, edges, cuts_total );
      }
      ...
      {
        stats << fmt::format(
            "[i] Delay    : Delay = {:8d}  Area = {:8d}  Edges = {:8d}  Cuts "
            "= "
            "{:8d}\n",
            delay, area, edges, cuts_total );
      }
  ...
  void compute_share_mapping( lut_cut_sort_type const sort, bool first )
  {
    ...
    /* round stats */
    {
      st.round_stats.push_back(
          fmt::format( "[i] AreaSh   : Delay = {:8d}  Area = {:8d}  Edges = "
                       "{:8d}  Cuts = {:8d}\n",
                       delay, area, edges, cuts_total ) );
    }
  }
  ...
  void expand_cuts()
  {
    ...
    std::string stats = fmt::format(
        "[i] Reduce   : Delay = {:8d}  Area = {:8d}  Edges = {:8d}  Cuts = "
        "{:8d}\n",
        delay, area, edges, cuts_total );
  ...
  void compute_required_time()
  {
    ...
    for ( auto it = topo_order.rbegin(); it != topo_order.rend(); ++it )
    {
      ...
      {
        node_match[leaf].required = std::min(
            node_match[leaf].required,
            node_match[index].required - cuts[index][0]->data.lut_delay );
      }
  ...
  void compute_share_mapping_init( bool first )
  {
    ...
    for ( auto i = 0u; i < node_match.size(); ++i )
    {
      node_match[i].required = UINT32_MAX >> 1;
      if ( !first )
        node_match[i].est_refs =
            ( 2.0 * node_match[i].est_refs + 1.0 * node_match[i].map_refs ) / 3.0;
      else
        node_match[i].est_refs = std::max(
            1.0, ( 1.0 * node_match[i].est_refs + 2.0 * node_match[i].map_refs ) /
                     3.0 );
  ...
  template<bool DO_AREA, bool ELA>
  void compute_choice_cut( node const& n, lut_cut_sort_type const sort,
                           bool preprocess )
  {
    auto index = ntk.node_to_index( n );
    auto& node_data = node_match[index];
    cut_t best_cut = cuts[index].best();
    // sort == lut_cut_sort_type::AREA;
    if ( node_data.map_refs > 0 )
    {
      cut_deref( cuts[index].best() );
    }

    // merge the cuts with choice nodes
    auto choice_node = ntk.get_equiv_node( n );
    auto choice_index = ntk.node_to_index( choice_node );
    while ( choice_node <= ntk.size() && choice_node != n )
    {
      if ( cuts[choice_index].size() == 0 )
      {
        choice_node = ntk.get_equiv_node( choice_node );
        choice_index = ntk.node_to_index( choice_node );
        continue;
      }
      for ( auto* cut : cuts[choice_index] )
      {
        if ( cut->size() <= 2 )
          continue;
        if ( ( *cut )->data.delay > node_data.required + epsilon )
          continue;
    ...
    cuts[index].limit( 199 );
    // update_cut_data_choice<DO_AREA, ELA>(n, sort);
    cuts[index].replace( 0, best_cut );
    add_unit_cut( index );
    cut_ref( cuts[index].best() );
  }
    ...
    if ( fanin > 1 && fanin <= ps.cut_enumeration_ps.fanin_limit )
    {
      ...
      foreach_mixed_radix_tuple(
          cut_sizes.begin(), cut_sizes.end(), [&]( auto begin, auto end )
          {
            ...
            if (!vcuts[0]->merge(*vcuts[1], new_cut,
            ...
            for (i = 2; i < fanin; ++i) {
              tmp_cut = new_cut;
              if (!vcuts[i]->merge(tmp_cut, new_cut,
                                   ps.cut_enumeration_ps.cut_size)) {
                return true; /* continue */
              }
            }
  ...
  template<bool DO_AREA, bool ELA>
  void update_cut_data_choice( node const& n, lut_cut_sort_type const sort )
  {
    auto index = ntk.node_to_index( n );
    auto& node_data = node_match[index];
    auto& node_cut_set = cuts[index];
  ...
  void update_cut_data_share( node const& n, lut_cut_sort_type const sort )
  {
    ...
    {
      node_match[leaf].required =
          std::min( node_match[leaf].required,
                    node_data.required - ( *best_cut )->data.lut_delay );
      node_match[leaf].map_refs++;
    }
  ...
  void expand_cuts_node( node const& n )
  {
    ...
    for ( auto const leaf : best_cut )
    {
      delay_update = std::max(
          delay_update, cuts[leaf][0]->data.delay + best_cut->data.lut_delay );
    }
    ...
    {
      delay_after = std::max(
          delay_after, cuts[leaf][0]->data.delay + new_cut->data.lut_delay );
    }
    ...
    if ( area_after <= area_before &&
         new_cut->data.delay <= node_data.required )
    {
  ...
  bool improve_cut( std::vector<uint32_t>& leaves )
  {
    ...
    if ( leaves.size() < ps.cut_enumeration_ps.cut_size &&
         improve_cut_expand1( leaves ) )
      return true;
    ...
    for ( auto it = leaves.begin(); it != leaves.end(); ++it )
    {
      ...
      ntk.foreach_fanin( ntk.index_to_node( *it ), [&]( auto const& f )
        ...
        if (!ntk.is_constant(ntk.get_node(f)) &&
            ntk.visited(ntk.get_node(f)) != ntk.trav_id())
          ++marked; } );
      ...
      ntk.foreach_fanin( ntk.index_to_node( *it ), [&]( auto const& f )
                         {
        if (ntk.is_constant(ntk.get_node(f))) return;
  ...
  bool improve_cut_expand1( std::vector<uint32_t>& leaves )
  {
    for ( auto it = leaves.begin(); it != leaves.end(); ++it )
    {
      ...
      /* test if expansion would increase the number of leaves by more than
       * 1*/
      int marked = 0;
      ntk.foreach_fanin( ntk.index_to_node( *it ), [&]( auto const& f )
                         {
  ...
  uint32_t cut_ref( cut_t const& cut )
  {
    ...
    for ( auto leaf : cut )
    {
      if ( ntk.is_ci( ntk.index_to_node( leaf ) ) ||
           ntk.is_constant( ntk.index_to_node( leaf ) ) )
      {
        continue;
      }
  ...
  void compute_cut_data( cut_t& cut, node const& n, bool recompute_cut_cost )
  {
    ...
    if ( recompute_cut_cost )
    {
      ...
      if constexpr ( StoreFunction )
      {
        ...
        else
        {
          if constexpr ( !std::is_same<LUTCostFn, lut_unitary_cost>::value )
          {
            if ( auto it = truth_tables_cost.find( cut->func_id );
                 it != truth_tables_cost.end() )
            {
          ...
          {
            std::tie( lut_area, lut_delay ) =
                lut_cost( truth_tables[cut->func_id] );
          }
        }
      }
      else
    ...
    else
    {
      ...
      for ( auto leaf : cut )
      {
        ...
        {
          area_flow +=
              best_leaf_cut->data.area_flow / node_match[leaf].est_refs;
          edge_flow +=
              best_leaf_cut->data.edge_flow / node_match[leaf].est_refs;
        }
        else
        {
  ...
  {
    auto& cut =
        cuts[index].add_cut( &index, &index ); /* fake iterator for emptyness */
  ...
  uint32_t compute_truth_table( uint32_t index,
                                ...
                                cut_t& res )
  {
    // stopwatch t( st.cut_enumeration_st.time_truth_table ); /* runtime
    // optimized */
    ...
    if ( ps.cut_enumeration_ps.minimize_truth_table &&
         !fast_support_minimization( tt_res, res ) )
    {
      const auto support = kitty::min_base_inplace( tt_res );
      if ( support.size() != res.size() )
      {
        auto tt_res_shrink =
            shrink_to( tt_res, static_cast<unsigned>( support.size() ) );
  ...
  void compute_mffcs_mapping()
  {
    ...
    ntk.foreach_co( [&]( auto const& f )
      ...
      if (ntk.is_ci(n) || ntk.is_constant(n)) return;
...

    {
      std::stringstream stats;
      stats << fmt::format(
          "[i] Area MFFC: Delay = {:8d}  Area = {:8d}  Edges = {:8d}  Cuts = "
          "{:8d}\n",
          delay, area, edges, cuts_total );
      st.round_stats.push_back( stats.str() );
    }
  ...
  void compute_mffc_mapping_node( node const& n )
  {
    ...
    for ( auto const& g : inner )
    {
      ntk.foreach_fanin( g, [&]( auto const& f )
                         {
        if (ntk.visited(ntk.get_node(f)) != ntk.trav_id() &&
            !ntk.is_constant(ntk.get_node(f))) {
    ...
    if constexpr ( StoreFunction )
    {
      ...
      node_to_value[ntk.get_node( ntk.get_constant( false ) )] =
          sim.compute_constant(
              ntk.constant_value( ntk.get_node( ntk.get_constant( false ) ) ) );
      if ( ntk.get_node( ntk.get_constant( false ) ) !=
           ntk.get_node( ntk.get_constant( true ) ) )
      {
        node_to_value[ntk.get_node( ntk.get_constant( true ) )] =
            sim.compute_constant(
                ntk.constant_value( ntk.get_node( ntk.get_constant( true ) ) ) );
      }
  ...
  void simulate_fc_rec( node const& n,
                        unordered_node_map<TT, Ntk>& node_to_value )
  {
  ...
  {
    klut_network res;
    mockturtle::node_map<mockturtle::signal<klut_network>, Ntk> node_to_signal(
        ntk );

    mockturtle::node_map<driver_type, Ntk> node_driver_type( ntk,
                                                             driver_type::none );

    /* opposites are filled for nodes with mixed driver types, since they have
       two nodes in the network. */
    std::unordered_map<node, mockturtle::signal<klut_network>> opposites;
    ...
    ntk.foreach_pi( [&]( auto n )
                    {
      mockturtle::signal<klut_network> res_signal;
      switch (node_driver_type[n]) {
        default:
        case driver_type::none:
        case driver_type::pos:
    ...
    for ( auto const& n : topo_order )
    {
      ...
      std::vector<mockturtle::signal<klut_network>> children;
  ...
  inline lut_info create_lut(
      node const& n,
      mockturtle::node_map<mockturtle::signal<klut_network>, Ntk>&
          node_to_signal,
      mockturtle::node_map<driver_type, Ntk> const& node_driver_type )
  {
    ...
    else
    {
      ...
      /* add constants */
      node_to_value[ntk.get_node( ntk.get_constant( false ) )] =
          kitty::dynamic_truth_table( best_cut.size() );
      ...
      for ( uint32_t leaf : best_cut )
      {
        ...
        kitty::create_nth_var(
            tt_leaf, ctr++,
            node_driver_type[ntk.index_to_node( leaf )] == driver_type::neg );
  ...
  void compute_function_rec(
      node const& n,
      unordered_node_map<kitty::dynamic_truth_table, Ntk>& node_to_value )
  {
    if ( ntk.visited( n ) == ntk.trav_id() )
    {
  ...
  void compute_balancing_cost_required( uint32_t index )
  {
    ...
    if ( cut.size() == 1 )
    {
      for ( auto l : cut )
      {
        node_match[l].required =
            std::min( node_match[l].required, node_match[index].required );
      }
    }
    ...
    auto priority_cmp = []( std::pair<uint32_t, uint16_t> const& a,
                            std::pair<uint32_t, uint16_t> const& b )
    {
      return a.first > b.first;
    };
    using cubes_queue2_t =
        std::priority_queue<std::pair<uint32_t, uint16_t>,
    ...
    compute_balancing_cost_propagate_required( connections, required, size,
                                               cut.size() );
    ...
    {
      node_match[l].required =
          std::min( node_match[l].required, required[ctr++] );
      assert( node_match[l].required >= cuts[l][0]->data.delay );
    }
  }

  template<typename Queue>
  inline void compute_balancing_cost_required_term(
      Queue& terms,
      std::array<std::pair<uint16_t, uint16_t>, max_sop_decomp_size>&
          connections,
      uint32_t& size )
  {
    while ( terms.size() != 1 )
  ...
  inline void compute_balancing_cost_propagate_required(
      std::array<std::pair<uint16_t, uint16_t>, max_sop_decomp_size> const&
          connections,
      std::array<uint32_t, max_sop_decomp_size>& required, uint32_t size,
      uint32_t leaves )
  {
    for ( auto i = size - 1; i >= leaves; --i )
    {
      uint32_t time = required[i] - 1;
      required[connections[i].first] =
          std::min( time, required[connections[i].first] );
      required[connections[i].second] =
          std::min( time, required[connections[i].second] );
    }
  }

private:
  Ntk& ntk;
  choice_lut_map_params const& ps;
  choice_lut_map_stats& st;
...
template<class Ntk, bool ComputeTruth = false,
         class LUTCostFn = lut_unitary_cost>
klut_network choice_lut_map( Ntk& ntk, choice_lut_map_params ps = {},
                             choice_lut_map_stats* pst = nullptr )
{
  ...
  static_assert( has_foreach_ci_v<Ntk>,
                 "Ntk does not implement the foreach_ci method" );
  ...
  detail::choice_lut_map_impl<Ntk, ComputeTruth, LUTCostFn> p( ntk, tps, st );
  klut = p.run();
...
template<class Ntk, bool StoreFunction = true,
         class LUTCostFn = lut_unitary_cost>
void choice_lut_map_inplace( Ntk& ntk, choice_lut_map_params const& ps = {},
                             choice_lut_map_stats* pst = nullptr )
{
  static_assert( is_network_type_v<Ntk>, "Ntk is not a network type" );
  static_assert( has_size_v<Ntk>, "Ntk does not implement the size method" );
  ...
  static_assert( has_fanout_size_v<Ntk>,
                 "Ntk does not implement the fanout_size method" );
  static_assert( has_clear_mapping_v<Ntk>,
                 "Ntk does not implement the clear_mapping method" );
  static_assert( has_add_to_mapping_v<Ntk>,
                 "Ntk does not implement the add_to_mapping method" );
  ...
  detail::choice_lut_map_impl<Ntk, StoreFunction, LUTCostFn> p( ntk, tps, st );
  p.run_inplace();
