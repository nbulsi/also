#ifndef CHOICE_EXACT_MAP
#define CHOICE_EXACT_MAP
 ...
 *
 * The data structure `choice_map_params` holds configurable parameters
 * with default arguments for `map`.
 */
struct choice_map_params {
  choice_map_params() {
    choice_cut_enumeration_ps.cut_limit = 49;
    choice_cut_enumeration_ps.cut_size = 4;
    choice_cut_enumeration_ps.minimize_truth_table = true;
  }
   ...
   */
  cut_enumeration_params choice_cut_enumeration_ps{};
 ...
 *
 * The data structure `choice_map_stats` provides data collected by running
 * `map`.
 */
struct choice_map_stats {
  /*! \brief Area result. */
  double area{0};
  ...
  cut_enumeration_stats choice_cut_enumeration_st{};
  ...
  void report() const {
    ...
    std::cout << fmt::format("[i] Area = {:>5.2f}; Delay = {:>5.2f};", area,
                             delay);
    if (power != 0)
...
struct choice_cut_match_tech {
  /* list of supergates matching the cut for positive and negative output phases
   */
  std::array<std::vector<supergate<NInputs>> const*, 2> supergates = {nullptr,
                                                                      nullptr};
...
class choice_tech_map_impl {
 ...
 public:
  explicit choice_tech_map_impl(
      ...
      : ntk(ntk),
        ...
        node_match(ntk.size()),
        matches(),
        switch_activity(ps.eswp_rounds
                            ? mockturtle::detail::switching_activity(
                                  ntk, ps.switching_activity_patterns)
                            : std::vector<float>(0)),
        cuts(fast_cut_enumeration<Ntk, CutSize, true, CutData>(
            ntk, ps.choice_cut_enumeration_ps, &st.choice_cut_enumeration_st)) {
  ...
  explicit choice_tech_map_impl(
      Ntk const& ntk, tech_library<NInputs, Configuration> const& library,
      std::vector<float> const& switch_activity, choice_map_params const& ps,
      choice_map_stats& st)
      : ntk(ntk),
        library(library),
        ps(ps),
  ...
  map_ntk_t run() {
    ...
    names_view<Ntk>(ntk).foreach_node(
        [this](auto n) { top_order.push_back(n); });
    if (ps.insert) {
      init_choice();
    }
  ...
  seq_map_ntk_t run_seq() {
    ...
    /* compute and save topological order */
    top_order.reserve(ntk.size());
    topo_view<Ntk>(ntk).foreach_node(
        [this](auto n) { top_order.push_back(n); });
 ...
 private:
  ...
  void init_choice() {
    ...
    ntk.foreach_node([&](auto const& n) {
      if (ntk.is_repr(n)) {
        ...
        auto choice_node = ntk.get_equiv_node(n);
        auto choice_index = ntk.node_to_index(choice_node);
        while (choice_node < ntk.size()) {
          choice_num++;
          for (auto* cut : cuts.cuts(choice_index)) {
            if (ntk.is_xor(n)) {
              continue;
            }
            auto cut_phase = ntk.phase(index) ^ ntk.phase(choice_index);
            if (cut->size() == 1) continue;
            auto new_cut = *cut;
            if (cut_phase) {
              new_cut->func_id ^= 1;
            }
            cuts.cuts(index).insert(new_cut);
          }
          choice_node = ntk.get_equiv_node(choice_node);
          choice_index = ntk.node_to_index(choice_node);
        }
        cuts.cuts(index).limit(199);
      }
    });
    std::cout << "choice node number = " << choice_num << std::endl;
  }
  ...
  void compute_matches() {
    ...
    ntk.foreach_gate([&](auto const& n) {
      ...
      std::vector<choice_cut_match_tech<NInputs>> node_matches;
      ...
      for (auto& cut : cuts.cuts(index)) {
        ...
        if (supergates_pos != nullptr || supergates_neg != nullptr) {
          choice_cut_match_tech<NInputs> match{{supergates_pos, supergates_neg},
  ...
  void create_lut_for_gate(NtkDest& res, klut_map& old2new, uint32_t index,
    ...
    std::vector<mockturtle::signal<klut_network>> children(gate->num_vars);
  ...
  template <class NtkDest>
  mockturtle::signal<klut_network> create_lut_for_gate_rec(
      NtkDest& res, composed_gate<NInputs> const& gate,
      std::vector<mockturtle::signal<klut_network>> const& children) {
    std::vector<mockturtle::signal<klut_network>> children_local(
        gate.fanin.size());
 ...
 private:
  ...
  std::vector<choice_node_match_tech<NInputs>> node_match;
  match_map matches;
...
template <class Ntk, unsigned CutSize = 5u,
          ...
          classification_type Configuration>
binding_view<klut_network> choice_map(
    Ntk const& ntk, tech_library<NInputs, Configuration> const& library,
    choice_map_params const& ps = {}, choice_map_stats* pst = nullptr) {
  ...
  choice_map_stats st;
  detail::choice_tech_map_impl<Ntk, CutSize, CutData, NInputs, Configuration> p(
      ntk, library, ps, st);
  auto res = p.run();

  st.time_total = st.time_mapping + st.choice_cut_enumeration_st.time_total;
 ...
 *
 * The function returns a sequential k-LUT network. Each LUT abstracts a gate of
...
template <class Ntk, unsigned CutSize = 5u,
          typename CutData = cut_enumeration_tech_map_cut, unsigned NInputs,
          classification_type Configuration>
binding_view<sequential<klut_network>> choice_seq_map(
    Ntk const& ntk, tech_library<NInputs, Configuration> const& library,
    choice_map_params const& ps = {}, choice_map_stats* pst = nullptr) {
  static_assert(is_network_type_v<Ntk>, "Ntk is not a network type");
  static_assert(has_size_v<Ntk>, "Ntk does not implement the size method");
  ...
  static_assert(has_get_node_v<Ntk>,
                "Ntk does not implement the get_node method");
  static_assert(has_foreach_co_v<Ntk>,
                "Ntk does not implement the foreach_co method");
  static_assert(has_foreach_ri_v<Ntk>,
                "Ntk does not implement the has_foreach_ri method");
  static_assert(has_foreach_pi_v<Ntk>,
                "Ntk does not implement the foreach_pi method");
  static_assert(has_foreach_po_v<Ntk>,
                "Ntk does not implement the has_foreach_po method");
  static_assert(has_foreach_ro_v<Ntk>,
                "Ntk does not implement the has_foreach_ro method");
  static_assert(has_foreach_node_v<Ntk>,
                "Ntk does not implement the foreach_node method");
  static_assert(has_fanout_size_v<Ntk>,
                "Ntk does not implement the fanout_size method");
...
struct cut_match_t {
  /* list of supergates matching the cut for positive and negative output phases
   */
  std::vector<exact_supergate<Ntk, NInputs>> const* supergates[2] = {nullptr,
                                                                     nullptr};
...
template <class NtkDest, unsigned CutSize, typename CutData, class Ntk,
          unsigned NInputs>
class choice_exact_map_impl {
 public:
 ...
 public:
  explicit choice_exact_map_impl(Ntk& ntk,
      ...
      : ntk(ntk),
        ...
        matches(),
        cuts(fast_cut_enumeration<Ntk, CutSize, true, CutData>(
            ntk, ps.choice_cut_enumeration_ps)) {
  ...
  NtkDest run() {
    ...
    while (iteration < ps.ela_rounds + ps.area_flow_rounds + 1) {
      ...
      if (ps.enable_logic_sharing &&
          iteration == ps.ela_rounds + ps.area_flow_rounds) {
 ...
 private:
  ...
  void init_choice() {
      ...
      if (ntk.is_repr(n)) {
        ...
        while (choice_node <= ntk.size()) {
          for (auto* cut : cuts.cuts(choice_index)) {
            ...
            bool cut_phase = ntk.phase(index) ^ ntk.phase(choice_index);
            if (cut->size() == 1) continue;
            auto new_cut = *cut;
            if (cut_phase) {
              new_cut->func_id ^= 1;
    ...
    ntk.foreach_gate([&](auto const& n) {
      ...
      for (auto& cut : cuts.cuts(index)) {
        ...
        auto const supergates_npn_neg =
            library.get_supergates(~std::get<0>(config));
  ...
  void compute_matches_dc() {
    ...
    ntk.foreach_gate([&](auto const& n) {
      ...
      std::vector<node<Ntk>> gates{
          collect_nodes(color_ntk, extended_leaves, roots)};
      ...
      const auto tts =
          simulate_nodes<kitty::static_truth_table<max_window_size>>(window_ntk,
                                                                     sim);
      ...
      for (auto& cut : cuts.cuts(index)) {
        ...
        for (auto const& l : *cut) {
          if (color_ntk.color(ntk.index_to_node(l)) !=
              color_ntk.current_color()) {
            containment = false;
            break;
          }
        }
        ...
        auto const dc_npn =
            apply_npn_transformation(~care, neg & ~(1 << NInputs), perm);
        const std::vector<exact_supergate<NtkDest, NInputs>>* supergates_npn =
            library.get_supergates(tt_npn, dc_npn, neg, perm);
        const std::vector<exact_supergate<NtkDest, NInputs>>*
            supergates_npn_neg =
                library.get_supergates(~tt_npn, dc_npn, neg_neg, perm_neg);
  ...
  std::pair<NtkDest, node_map<mockturtle::signal<NtkDest>, Ntk>>
  initialize_dest() {
    node_map<mockturtle::signal<NtkDest>, Ntk> old2new(ntk);
    NtkDest dest;
    ...
    ntk.foreach_pi([&](auto const& n) { old2new[n] = dest.create_pi(); });

    if constexpr (has_foreach_ro_v<Ntk>) {
      ntk.foreach_ro([&](auto const& n) { old2new[n] = dest.create_ro(); });
    }

    return {dest, old2new};
  }

  void finalize_cover(NtkDest& res,
                      node_map<mockturtle::signal<NtkDest>, Ntk>& old2new) {
    if (!ps.enable_logic_sharing || iteration == ps.area_flow_rounds + 1) {
      ...
      ntk.foreach_node([&](auto const& n) {
        ...
        auto f = cleanup_dangling(topo, res, children.begin(), children.end())
                     .front();
    ...
    if constexpr (has_foreach_ri_v<Ntk>) {
      ntk.foreach_ri([&](auto const& f) {
        res.create_ri(ntk.is_complemented(f) ? res.create_not(old2new[f])
                                             : old2new[f]);
      });
    }
  ...
  bool set_mapping_refs() {
    ...
    for (auto it = top_order.rbegin(); it != top_order.rend(); ++it) {
      ...
      if (node_data.best_supergate[use_phase] == nullptr) {
        /* Library is not complete, mapping is not possible */
        std::cerr << "[i] MAP ERROR: library is not complete, impossible to "
                     "perform mapping"
                  << std::endl;
        st.mapping_error = true;
        return false;
  ...
  void compute_required_time() {
    ...
    while (i-- > 0u) {
      ...
      if (ntk.is_ci(n) || ntk.is_constant(n)) break;

      if (node_match[i].map_refs[2] == 0) continue;
      ...
      if (node_data.same_match || node_data.map_refs[use_phase] > 0) {
        ...
        for (auto leaf : best_cut) {
          auto phase =
              (node_data.phase[use_phase] >> match.permutation[ctr]) & 1;
          node_match[leaf].required[phase] =
              std::min(node_match[leaf].required[phase],
                       node_data.required[use_phase] -
                           supergate->tdelay[match.permutation[ctr]]);
          ctr++;
        }
      }
      ...
      if (!node_data.same_match && node_data.map_refs[other_phase] > 0) {
        ...
        for (auto leaf : best_cut) {
          auto phase =
              (node_data.phase[other_phase] >> match.permutation[ctr]) & 1;
          node_match[leaf].required[phase] =
              std::min(node_match[leaf].required[phase],
                       node_data.required[other_phase] -
                           supergate->tdelay[match.permutation[ctr]]);
          ctr++;
        }
      }
  ...
  void match_phase(node<Ntk> const& n, uint8_t phase) {
    ...
    auto& node_data = node_match[index];
    auto& cut_matches = matches[index];
    exact_supergate<NtkDest, NInputs> const* best_supergate =
        node_data.best_supergate[phase];
    ...
    if (best_supergate != nullptr) {
      ...
      for (auto pin = 0u; pin < NInputs; pin++) {
        float arrival_pin =
            node_match[children[pin]].arrival[(best_phase >> pin) & 1] +
            best_supergate->tdelay[pin];
        best_arrival = std::max(best_arrival, arrival_pin);
      }
    }
    ...
    for (auto& cut : cuts.cuts(index)) {
      ...
      for (auto const& gate : *supergates.supergates[phase]) {
        ...
        for (auto pin = 0u; pin < NInputs; pin++) {
          float arrival_pin =
              node_match[children[pin]].arrival[(complement >> pin) & 1] +
              gate.tdelay[pin];
          worst_arrival = std::max(worst_arrival, arrival_pin);
        }
  ...
  bool compute_exact_area_aggressive(
      NtkDest& res, node_map<mockturtle::signal<NtkDest>, Ntk>& old2new) {
    ...
    for (auto const& n : top_order) {
      ...
      if (node_data.best_supergate[0] == nullptr) {
        ...
        if (node_data.map_refs[2]) {
          ...
          mockturtle::detail::recursive_ref<NtkDest>(res, res.get_node(sig1));
        }
        old2new[n] = sig1;
        continue;
      } else if (node_data.best_supergate[1] == nullptr) {
        ...
        if (node_data.map_refs[2]) {
          ...
          mockturtle::detail::recursive_ref<NtkDest>(res, res.get_node(sig0));
        }
        old2new[n] = sig0;
        continue;
      }
      ...
      if (use_zero && use_one) {
        auto size_zero = cuts.cuts(index)[node_data.best_cut[0]].size();
        auto size_one = cuts.cuts(index)[node_data.best_cut[1]].size();
        if (compare_map<true>(worst_arrival_nneg, worst_arrival_npos,
                              node_data.flows[0], node_data.flows[1], size_zero,
                              size_one))
          use_one = false;
        else
          use_zero = false;
    ...
    if (ps.verbose) {
      ...
      std::stringstream stats{};
      stats << fmt::format(
          "[i] Area RW  : Delay = {:>12.2f}  Area = {:>12.2f}  {:>5.2f} %\n",
          delay, area, area_gain);
      st.round_stats.push_back(stats.str());
    }
  ...
  mockturtle::signal<NtkDest> match_phase_exact_aggressive(
      depth_view<NtkDest>& res,
      node_map<mockturtle::signal<NtkDest>, Ntk>& old2new, node<Ntk> const& n,
      uint8_t phase) {
    mockturtle::signal<NtkDest> best_signal = res.get_constant(false);
    ...
    if (best_supergate != nullptr) {
      ...
      auto f =
          cleanup_dangling(topo, res, children.begin(), children.end()).front();
      ...
      if (!node_data.same_match && node_data.map_refs[phase]) {
        best_area =
            mockturtle::detail::recursive_ref<NtkDest>(res, res.get_node(f));
        mockturtle::detail::recursive_deref<NtkDest>(res, res.get_node(f));
        best_exact_area = cut_deref(cuts.cuts(index)[best_cut], n, phase);
      } else {
        best_area =
            mockturtle::detail::recursive_ref<NtkDest>(res, res.get_node(f));
    ...
    for (auto& cut : cuts.cuts(index)) {
      ...
      if ((*cut)->data.ignore || (rewrite_count > ps.logic_sharing_cut_limit &&
                                  cut_index != best_cut)) {
        ++cut_index;
        continue;
      }
    ...
    if (!node_data.same_match && node_data.map_refs[phase]) {
      mockturtle::detail::recursive_ref<NtkDest>(res,
                                                 res.get_node(best_signal));
      best_exact_area = cut_ref(cuts.cuts(index)[best_cut], n, phase);
    }
    return best_signal;
  }
  ...
  void match_drop_phase(node<Ntk> const& n, unsigned area_margin_factor) {
    ...
    if constexpr (!DO_AREA) {
      ...
      if (!use_zero && !use_one) {
        ...
        node_data.flows[2] =
            (node_data.flows[0] + node_data.flows[1]) / node_data.est_refs[2];
        node_data.flows[0] = node_data.flows[0] / node_data.est_refs[0];
        node_data.flows[1] = node_data.flows[1] / node_data.est_refs[1];
        return;
      }
      ...
      /* check if both phases + inverter meet the required time */
      use_zero = worst_arrival_nneg < node_data.required[1] + epsilon -
                                          area_margin_factor * lib_inv_delay;
      use_one = worst_arrival_npos < node_data.required[0] + epsilon -
                                         area_margin_factor * lib_inv_delay;
    }
  ...
  inline float cut_leaves_flow(cut_t const& cut, node<Ntk> const& n,
                               uint8_t phase) {
    float flow{0.0f};
    auto const& node_data = node_match[ntk.node_to_index(n)];
    ...
    for (auto leaf : cut) {
      uint8_t leaf_phase =
          (node_data.phase[phase] >> match.permutation[ctr++]) & 1;
      flow += node_match[leaf].flows[leaf_phase];
    }
  ...
  float cut_ref(cut_t const& cut, node<Ntk> const& n, uint8_t phase) {
    ...
    for (auto leaf : cut) {
      ...
      if (ntk.is_constant(ntk.index_to_node(leaf))) {
        ...
        /* reference PIs, add inverter cost for negative phase */
        if (leaf_phase == 1u) {
          if (node_match[leaf].map_refs[1]++ == 0u) count += lib_inv_area;
        } else {
          ++node_match[leaf].map_refs[0];
        }
        ++ctr;
      ...
      if (node_match[leaf].same_match) {
        ...
        if (node_match[leaf].map_refs[2]++ == 0u) {
          count +=
              cut_ref(cuts.cuts(leaf)[node_match[leaf].best_cut[leaf_phase]],
                      ntk.index_to_node(leaf), leaf_phase);
        }
      } else {
        ++node_match[leaf].map_refs[2];
  ...
  float cut_deref(cut_t const& cut, node<Ntk> const& n, uint8_t phase) {
    ...
    for (auto leaf : cut) {
      ...
      if (node_match[leaf].same_match) {
        ...
        if (--node_match[leaf].map_refs[2] == 0u) {
          count +=
              cut_deref(cuts.cuts(leaf)[node_match[leaf].best_cut[leaf_phase]],
                        ntk.index_to_node(leaf), leaf_phase);
        }
      } else {
        --node_match[leaf].map_refs[2];
  ...
  template <bool DO_AREA>
  inline bool compare_map(float arrival, float best_arrival, float area_flow,
                          float best_area_flow, uint32_t size,
                          uint32_t best_size) {
    if constexpr (DO_AREA) {
      if (area_flow < best_area_flow - epsilon) {
        return true;
 ...
 private:
  ...
  std::unordered_map<uint32_t, std::vector<cut_match_t<NtkDest, NInputs>>>
      matches;
  network_cuts_t cuts;
};
 ...
 * `cut_enumeration_exact_map_cut`). The argument is similar to the `CutData`
 * argument in `cut_enumeration`, which can specialize the cost function to
 * select priority cuts and store additional data. The default argument gives
 * priority firstly to area flow, then delay, and lastly to the cut size. The
 * type passed as `CutData` must implement the following four fields:
 *
 * - `uint32_t delay`
 * - `float flow`
 * - `uint8_t match_index`
 ...
 *
 * See
 * `include/mockturtle/algorithms/cut_enumeration/cut_enumeration_exact_map_cut.hpp`
 * for one example of a CutData type that implements the cost function that is
 * used in the technology mapper.
 *
 * The function takes the size of the cuts in the template parameter `CutSize`.
 *
 * The function returns a mapped network representation generated using the
 * exact synthesis entries in the `exact_library`. This function supports also
 * sequential networks.
 *
 * **Required network functions:**
 * - `size`
 ...
 * \param pst Mapping statistics
 */
template <class Ntk, unsigned CutSize = 4u,
          typename CutData = cut_enumeration_exact_map_cut, class NtkDest,
          unsigned NInputs>
NtkDest choice_exact_map(Ntk& ntk,
                         exact_library<NtkDest, NInputs> const& library,
                         choice_map_params const& ps = {},
                         choice_map_stats* pst = nullptr) {
  static_assert(is_network_type_v<Ntk>, "Ntk is not a network type");
  static_assert(has_size_v<Ntk>, "Ntk does not implement the size method");
  static_assert(has_is_ci_v<Ntk>, "Ntk does not implement the is_ci method");
  static_assert(has_is_constant_v<Ntk>,
  ...
  static_assert(has_fanout_size_v<Ntk>,
                "Ntk does not implement the fanout_size method");
  static_assert(has_incr_value_v<NtkDest>,
                "Ntk does not implement the incr_value method");
  static_assert(has_decr_value_v<NtkDest>,
                "Ntk does not implement the decr_value method");
  static_assert(has_foreach_ri_v<Ntk> == has_create_ri_v<NtkDest>,
                "Ntk and NtkDest networks are not both sequential");
  static_assert(has_foreach_ro_v<Ntk> == has_create_ro_v<NtkDest>,
                "Ntk and NtkDest networks are not both sequential");

  choice_map_stats st;
  detail::choice_exact_map_impl<NtkDest, CutSize, CutData, Ntk, NInputs> p(
      ntk, library, ps, st);
  auto res = p.run();
